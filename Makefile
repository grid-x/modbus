...
.PHONY: test
test:
	socat -d -d pty,raw,echo=0 tcp-listen:5022,reuseaddr,fork & diagslave -m ascii /dev/pts/0 & diagslave -m tcp -p 5020 & diagslave -m enc -p 5021 & go test -run TCP -v $(shell glide nv)
